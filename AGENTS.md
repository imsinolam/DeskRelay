...
- **ClawBot 的裸“任务”必须在 daemon 尚未选中 active adapter 时仍可用，并且只聚合当前已连接或电脑上明确正在运行的终端。** `--idle-start` 会让 DeskRelay 在线但 `activeAdapter` 为空，如果先执行“尚未选择终端”保护就会吞掉原本的聚合入口；同时枚举所有已安装终端会混入未运行终端的历史任务并造成不必要探测，因此任务列表要先于 active-slot 校验处理，只做轻量、只读枚举且绝不能为列任务自动启动 Agent、恢复持久化会话或调用桌面端 `openThread`，否则用户仅查看列表也会被强制跳到某条 ChatGPT 任务。默认页还要先保证每个有任务的运行终端至少出现一条，再按更新时间补足，否则任务量大的单一终端会挤掉其他终端，让聚合列表视觉上再次退化成单终端列表。
