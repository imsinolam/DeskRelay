...
- **移动网页必须把消息、进展、审批请求和审批结果视为同一条任务时间线，并按真实发生时间稳定交错渲染。** 只按类型分区或只凭 turnId 把审批追加到轮次末尾，会让用户误以为所有审批都发生在最终回答之后；从 Agent 原始记录保留时间戳，审批结果同时保留请求时间，并仅对无时间的旧数据使用同轮次兼容回退，才能保证刷新、分页和历史恢复后的顺序一致。
