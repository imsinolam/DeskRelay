...
- **移动网页的跨刷新秒开缓存不能等待电脑在线检测或实时任务接口；同源浏览器曾成功认证且可信标记仍有效时，应先同步展示缓存，再用每个 adapter + threadId 的内容修订号和只读取最新消息/运行态的轻量接口校验，修订号变化后才拉完整消息。** 公网 Relay 在电脑离线时仍能返回静态页面但无法转发实时 API，若把缓存恢复放在 `/health` 或 `/api/auth/status` 之后，用户仍会停在等待页；缓存必须带 24 小时 TTL、adapter + threadId 隔离、退出清除，并排除图片 Base64、审批内容和令牌，认证明确失效时再隐藏缓存。轻量校验还应在唯一的小状态位显示“检查更新 / 同步更新 / 已是最新”，让用户知道预加载内容是否新鲜，同时避免无变化时重复传输完整会话。
