...
function isJavascriptRuntimeCommand(command: string): boolean {
  return ["node", "node.exe", "bun", "bun.exe"].includes(
...
export function isWeRelayDaemonCommandLine(commandLine: string): boolean {
  ...
  let runtimeIndex = 0;
  if (commandBasename(tokens[0] ?? "") === "env") {
    runtimeIndex = tokens.findIndex((token, index) =>
      index > 0 && isJavascriptRuntimeCommand(token)
    );
  }
  if (
    runtimeIndex < 0 ||
    !isJavascriptRuntimeCommand(tokens[runtimeIndex] ?? "")
  ) {
    return false;
  }
