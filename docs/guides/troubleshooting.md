...
本文记录 DeskRelay 的常见问题、已知限制和排查入口，包括 Agent 安装、状态文件、网络发送、会话同步、移动网页和审批行为。
...
如果设置了 `DESKRELAY_DATA_DIR`，状态文件和日志会写入对应目录。运行时环境变量的完整说明见 [运行配置](configuration.md)。

未设置自定义数据目录时，登录凭据默认写入：

```text
~/.deskrelay/account.json
```

登录成功后会清理旧的 `sync_buf.txt` 和 `context_tokens.json`，避免旧会话状态污染新的登录状态。

从 1.x 升级时，DeskRelay 会优先从旧的 `~/.cli-bridge` 一次性复制数据到 `~/.deskrelay`；更早的 `~/.claude/channels/wechat` 仍可作为迁移来源。迁移包括登录凭据、同步游标、上下文 token、工作区状态和已接收附件；已有的 2.0 文件不会被覆盖，旧目录也会原样保留但不再作为活动目录。旧运行锁不会迁移，旧 `bridge.log` 会保存为 `legacy-bridge.log`，避免覆盖新的运行日志。
...
| `daemon-endpoint.json` | 当前 `deskrelay-daemon` 的本地 IPC endpoint |
| `inbound-attachments/<date>/` | 微信入站图片和普通文件的本地保存目录 |
| `workspaces/<workspace-key>/bridge-state.json` | 当前工作区状态、会话和 adapter 状态 |
| `workspaces/<workspace-key>/daemon-state.json` | daemon 当前活动适配器、已选 Codex 桌面任务和移动版访问密钥；用于进程重启后恢复 |
...
## `deskrelay-<agent>` 提示找不到 bridge

通常原因是：

- 还没有先启动对应的 `deskrelay-bridge-*`；
- bridge 与 companion 不在同一个工作目录；
- 当前工作区 endpoint 文件来自旧进程或已经失效。

建议先确认两个终端在同一目录。如果不想手动分两个终端，可以使用对应的 `deskrelay-<agent>-start`，例如 `deskrelay-codex-start`、`deskrelay-grok-start`、`deskrelay-codebuddy-start`、`deskrelay-reasonix-start` 或 `deskrelay-opencode-start`。这些启动器会自动复用同目录 daemon，或在没有 daemon 时启动 bridge 与同一个可见 owner。
...
这表示 DeskRelay 启动子进程时找不到对应命令，不是任务列表故障。先在启动 daemon 的同一终端中检查：
...
Windows PowerShell 使用 `Get-Command <agent>`。如果直接运行 Agent 也失败，请先按供应方方式完成安装和登录；如果直接运行成功而 DeskRelay 失败，通常是 LaunchAgent/systemd/IDE 没有继承同一 `PATH`。

完整步骤见 [Agent 安装与配置](agent-setup.md)。
...
```bash
PKG="$(npm root -g)/deskrelay/node_modules/node-pty"
...
4. **多个进程在抢同一账号的消息**：残留的 `deskrelay-daemon`/bridge 进程、或在 Claude Code 中同时配置了 wechat MCP server 时，消息会被另一个进程先取走。运行 `--doctor` 查看守护进程与桥接锁状态，关掉多余进程。
5. **同一账号在另一台机器重新扫码登录**：旧机器的会话会失效（`errcode=-14 session timeout`），需要重新运行 `deskrelay-setup`。

## DeskRelay 移动网页打不开

### 局域网模式

1. 确认手机和电脑连接同一个可互访局域网；移动数据、访客 Wi-Fi 或客户端隔离会阻止访问。
2. 确认 `~/.deskrelay/bridge.log` 中存在 `codex_mobile_started`，该日志名为历史兼容，页面本身支持多个 Agent。
3. 确认 macOS/Windows/Linux 防火墙允许 Node.js 接收入站连接。
4. 默认从端口 `4396` 开始监听；冲突时可能使用 `4397` 等后续端口。
5. 电脑切换 Wi-Fi 或局域网地址后，重启 daemon 并使用新的任务链接。
6. 某些微信内置浏览器会限制局域网 HTTP，可尝试系统浏览器。

### 公网 Relay 模式

1. 公网 `/health` 必须返回 `ok: true`；
2. 电脑连接后 `deviceOnline` 应为 `true`；
3. 检查电脑上的 `DESKRELAY_RELAY_URL`、设备 ID 和设备密钥；
4. 检查服务器的 `DESKRELAY_RELAY_DEVICE_ID/TOKEN` 是否一致；
5. 确认 Relay 只监听服务器 `127.0.0.1`，Nginx/Caddy 提供公网 HTTPS；
6. 页面能打开但任务 API 超时时，检查电脑是否睡眠、daemon 是否退出以及反向代理超时。

完整部署见 [移动网页与公网访问](remote-access.md)。
...
1. 先确认本地 `deskrelay-codex` 是否真的仍在执行任务；
2. 必要时在微信发送 `/stop`；
3. 检查 `~/.deskrelay/bridge.log` 和当前工作区的 `bridge-state.json`。

## 本地 `/resume` 后微信不同步

请优先确认对应的 `deskrelay-bridge-<agent>` 与 `deskrelay-<agent>` 是否都已重启到同一版本。
...
如果希望关闭所有自动放行（每个权限请求都转发到微信确认），设置环境变量 `DESKRELAY_STRICT_APPROVAL=1` 后重启桥接。
...
- `deskrelay` 当前绑定启动工作区，暂不支持从微信切换到另一个本地目录。
