...
在 ClawBot 里发送图片时，DeskRelay 会先暂存图片，不会立刻给 Agent 下任务。可以连续发送多张图片，再补充任务说明：说明超过 10 个字时自动把全部图片和文字作为一条任务发送；说明不超过 10 个字时会询问是否完整，回复 `1` 发送，回复 `2` 保留图片并重新书写。发送“取消图片”可以放弃本次图片任务。
