...
如果设置了 `DESKRELAY_DATA_DIR`，状态文件和日志会写入对应目录。运行时环境变量的完整说明见 [运行配置](运行配置.md)。
...
如果命令仍不存在，请检查 npm 全局 bin 目录是否已加入 `PATH`。使用源码仓库提供全局命令的方式见 [开发说明](../开发协作/开发与测试.md#全局命令开发验证)。
...
完整步骤见 [Agent 安装与配置](Agent安装与配置.md)。
...
首次局域网使用见 [5 分钟启用局域网移动网页](局域网移动网页快速开始.md)；公网部署与逐项验收见 [公网 Relay Agent 配置与验收](公网Relay配置与验收.md)；原理和安全边界见 [局域网与公网访问](../架构设计/局域网与公网访问.md)。
