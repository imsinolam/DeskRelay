...
本文记录 WeRelay 的常见问题、已知限制和排查入口，包括 Agent 安装、状态文件、网络发送、会话同步、移动网页和审批行为。
...
如果设置了 `WERELAY_DATA_DIR`，状态文件和日志会写入对应目录。运行时环境变量的完整说明见 [运行配置](运行配置.md)。

未设置自定义数据目录时，登录凭据默认写入：

```text
~/.werelay/account.json
```

登录成功后会清理旧的 `sync_buf.txt` 和 `context_tokens.json`，避免旧会话状态污染新的登录状态。

从 DeskRelay 升级时，WeRelay 会优先从旧的 `~/.deskrelay` 一次性复制数据到 `~/.werelay`；缺失的数据再从更早的 `~/.cli-bridge` 或 `~/.claude/channels/wechat` 补齐。迁移包括登录凭据、同步游标、上下文 token、工作区状态和已接收附件；已有的 WeRelay 文件不会被覆盖，旧目录也会原样保留但不再作为活动目录。旧运行锁不会迁移，旧 `bridge.log` 会保存为 `legacy-bridge.log`，避免覆盖新的运行日志。
...
| `daemon-endpoint.json` | 当前 `werelay-daemon` 的本地 IPC endpoint |
...
## `werelay-<agent>` 提示找不到 bridge

通常原因是：

- 还没有先启动对应的 `werelay-bridge-*`；
- bridge 与 companion 不在同一个工作目录；
- 当前工作区 endpoint 文件来自旧进程或已经失效。

建议先确认两个终端在同一目录。如果不想手动分两个终端，可以使用对应的 `werelay-<agent>-start`，例如 `werelay-codex-start`、`werelay-grok-start`、`werelay-codebuddy-start`、`werelay-reasonix-start` 或 `werelay-opencode-start`。这些启动器会自动复用同目录 daemon，或在没有 daemon 时启动 bridge 与同一个可见 owner。
...
这表示 WeRelay 启动子进程时找不到对应命令，不是任务列表故障。先在启动 daemon 的同一终端中检查：
...
Windows PowerShell 使用 `Get-Command <agent>`。如果直接运行 Agent 也失败，请先按供应方方式完成安装和登录；如果直接运行成功而 WeRelay 失败，通常是 LaunchAgent/systemd/IDE 没有继承同一 `PATH`。
...
```bash
PKG="$(npm root -g)/werelay/node_modules/node-pty"
...
4. **多个进程在抢同一账号的消息**：残留的 `werelay-daemon`/bridge 进程、或在 Claude Code 中同时配置了 wechat MCP server 时，消息会被另一个进程先取走。运行 `--doctor` 查看守护进程与桥接锁状态，关掉多余进程。
5. **同一账号在另一台机器重新扫码登录**：旧机器的会话会失效（`errcode=-14 session timeout`），需要重新运行 `werelay-setup`。
...
2. 确认 `~/.werelay/bridge.log` 中存在 `codex_mobile_started`，该日志名为历史兼容，页面本身支持多个 Agent。
...
3. 检查电脑上的 `WERELAY_RELAY_URL`、设备 ID 和设备密钥；
4. 检查服务器的 `WERELAY_RELAY_DEVICE_ID/TOKEN` 是否一致；
...
1. 先确认本地 `werelay-codex` 是否真的仍在执行任务；
2. 必要时在微信发送 `/stop`；
3. 检查 `~/.werelay/bridge.log` 和当前工作区的 `bridge-state.json`。

## 本地 `/resume` 后微信不同步

请优先确认对应的 `werelay-bridge-<agent>` 与 `werelay-<agent>` 是否都已重启到同一版本。
...
如果希望关闭所有自动放行（每个权限请求都转发到微信确认），设置环境变量 `WERELAY_STRICT_APPROVAL=1` 后重启桥接。
...
- `werelay` 当前绑定启动工作区，暂不支持从微信切换到另一个本地目录。
