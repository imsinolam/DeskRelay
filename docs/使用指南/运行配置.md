...
- `--no-open`：不自动打开可见终端窗口。
- DeskRelay 后台启动和自动恢复默认不会启动 ChatGPT、WorkBuddy 等桌面应用；用户在网页或 ClawBot 中明确选择对应终端时，视为一次显式操作，可以按需打开或重新接入该应用。若需要在 daemon 启动阶段就自动打开桌面应用，必须显式增加 `--open-desktop-apps`。
